...
function  printQuote() {
    ...
    const quoteContainer = document.querySelector('.text-area')
    if (quoteContainer.contains(document.getElementById('quote'))) {
        document.querySelectorAll('.text-area > p').forEach((e) => {
            e.remove()
        })
    }
    ...
    pSource.setAttribute('id', 'source')
    pQuote.appendChild(document.createTextNode(quote.quote))
    pSource.appendChild(document.createTextNode('- ' + quote.source))
    quoteContainer.appendChild(pQuote)
    quoteContainer.appendChild(pSource)
}

function countDown() {
    let firstTime = true;
    console.log(firstTime)
    const counter = document.querySelector('#count-down > span')
    let seconds = quoteInternal/1000;
...
nextBtn.addEventListener('click', () => {
    if (nextBtn.innerText == 'Start') {
        ...
        document.querySelector('#count-down > span').innerText = 15
    }
})
